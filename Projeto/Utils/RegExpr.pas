...
const
 ...
 {*
   Holds around 40 error messages. Returns message for error with ID = AErrorID.

   @param Enumerated error type
   @return One line error message for AErrorID
 }
...
/// Returns ID of last error, 0 if no errors (unusable if Error method raises exception)
...
class
  /// Returns the Major version of TRegExpr, for example v0.944 returns 0.
  function TRegExpr.VersionMajor : integer; //###0.944
  begin
    Result := TRegExprVersionMajor;
  end
; { of class function TRegExpr.VersionMajor
--------------------------------------------------------------}

class
  /// Returns the minor version of TRegExpr, for example v0.944 returns 944.
  function TRegExpr.VersionMinor : integer; //###0.944
  begin
    Result := TRegExprVersionMinor;
  end
; { of class function TRegExpr.VersionMinor
--------------------------------------------------------------}
/// Creates a an empty RE object and sets default parameter settings
...
--------------------------------------------------------------}
/// Destroys the RE object and frees memory
...
/// Inverts the case of Ch (upper to lower or lower to upper). Uses system local setings.
...
--------------------------------------------------------------}
/// Calls CompileRegExpr to compile a regular expression into internal code
...
{*
  Tests AInputString against self's regular expression. AInputString will be
  stored into the InputString property. 
  @return True if AInputString matches pattern
}
...
{*
  Tests current text of InputString property against self's regular expression
...
{*
  To start pattern matching somewhere else than the beginning of the input string
  Same as ExecPos method

  @param AOffset Starting character position, the first position is 1
  @return True if R.E. pattern matched, else false
}
...
{*
  To find match for InputString starting from AOffset position

  @param AOffset Starting character position, the first position of InputString is 1
  @return True if R.E. pattern matched, else false
}
...
{*
  Finds next match in text of InputString property

  @return True if another match was found, else false.
  @comment Raises exception if used without preceeding successful call to
  Exec* (Exec, ExecPos, ExecNext). So You must use something like
  if Exec (InputString) then repeat until not ExecNext;
}
...
/// Getter function for InputString property
...
/// Setter function for InputString property
...
{*
   Perform substitutions after a regexp match. Returns ATemplate with '$&' or '$0'
   replaced by whole r.e. occurence and '$n' replaced by occurence of subexpression #n.
}
...
{*
  Copies the text of AInputStr between R.E. matches as separate lines into APieces,
  (splits AInputStr into APieces by r.e. occurence). Internally calls Exec[Next].
}
...
{*
  Replaces any matches with the contents of AInputStr and returns the result.
  If AUseSubstitution is true then AReplaceStr will be used as a template
...
{*
  Replaces any matches with the contents of AInputStr
...
{*
  Replaces any matches with the contents of AInputStr
  and returns the result. Call-back function is to implement complex functionality.
  If AUseSubstitution is true then AReplaceStr will be used as a template
  for substitution methods.
}
...
{*  printable representation of regexp opcode in vaguely comprehensible form
    @return String containing the text version of the regexp pcode
    @comment Probably msost usefull to unit developer as debugging tool
}
