unit uDadosLaudo;

interface

uses
  System.SysUtils, System.Classes, Data.DB, DBAccess, Uni, UniProvider,
  MySQLUniProvider, MemDS;

type
    /// <url>element://model:project::Gestor/delphi:e_field:src:Gestor:uDadosLaudo.TDadosLaudo.qryLaudo</url>
    TDadosLaudo = class(TDataModule)
        /// <url>element://model:project::Gestor/delphi:e_class:src:Gestor:uDadosLaudo.TDadosLaudo</url>
        qryLaudo: TUniQuery;
        qryLaudocodigo: TIntegerField;
    qryLaudodata_classificacao: TDateField;
    qryLaudomunicipio_classificacao: TStringField;
    qryLaudolocal_classificao: TStringField;
    qryLaudonumero_contrato: TStringField;
    qryLaudolacre: TStringField;
    qryLaudoorigem: TStringField;
    qryLaudodestino: TStringField;
    qryLaudoplaca: TStringField;
    qryLaudopeso_liquido: TFloatField;
    qryLaudopeso_tara: TFloatField;
    qryLaudopeso_bruto: TFloatField;
    qryLaudonota_fiscal: TIntegerField;
    qryLaudoproduto: TStringField;
    qryLaudoveiculo_vistoria: TStringField;
    qryLaudomotorista: TStringField;
    qryLaudocpf_motorista: TStringField;
    qryLaudotipo_produto: TStringField;
    qryLaudoqueimados: TFloatField;
    qryLaudoardidos: TFloatField;
    qryLaudomofados: TFloatField;
    qryLaudoavariados: TFloatField;
    qryLaudoimpurezas: TFloatField;
    qryLaudoamassados: TFloatField;
    qryLaudoumidade: TFloatField;
    qryLaudoesverdeados: TFloatField;
    qryLaudopicados: TFloatField;
    qryLaudofermentados: TFloatField;
    qryLaudoinsetos: TStringField;
    qryLaudosementes_toxicas: TStringField;
    qryLaudoclassificador: TStringField;
    qryLaudodata_emissao: TDateField;
    qryLaudolaudo: TStringField;
    qryLaudosituacao: TStringField;
    qryLaudocnpj: TStringField;
    qryLaudocliente: TStringField;
    MySQL: TMySQLUniProvider;
    Conexao: TUniConnection;
    qryExecute: TUniQuery;
  private
    { Private declarations }
  public
    { Public declarations }
  end;

var
  DadosLaudo: TDadosLaudo;

implementation

{%CLASSGROUP 'Vcl.Controls.TControl'}

{$R *.dfm}

end.
