...
begin
  ...
  qryUsuariosSUPERVISOR.Value := 'N';
  qryUsuariosFK_VENDEDOR.Value := 1;
...
begin
  if trim(qryUsuariosLOGIN.Text) = '' then
  begin
    ShowMessage('Digite o usuário!');
    exit;
  end;

  if trim(qryUsuariosSENHA.Text) = '' then
  begin
    ShowMessage('Digite a senha!');
    exit;
  end;

  qryUsuariosSENHA.Value := dados.Crypt('C', qryUsuariosSENHA.Value);
