object frmTransfComanda: TfrmTransfComanda
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Transferir Comanda'
  ClientHeight = 301
  ClientWidth = 428
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 109
    Top = 137
    Width = 213
    Height = 21
    Caption = 'Selecione Comanda de Destino'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Segoe UI Semilight'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel
    Left = 110
    Top = 70
    Width = 211
    Height = 21
    Caption = 'Selecione Comanda de Origem'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Segoe UI Semilight'
    Font.Style = []
    ParentFont = False
  end
  object Panel5: TPanel
    Left = 0
    Top = 0
    Width = 428
    Height = 49
    Align = alTop
    BevelOuter = bvNone
    Color = 5592405
    ParentBackground = False
    TabOrder = 0
    DesignSize = (
      428
      49)
    object Label11: TLabel
      Left = 471
      Top = 22
      Width = 188
      Height = 17
      Anchors = [akTop, akRight]
      Caption = 'Relat'#243'rio de Vendas no Per'#237'odo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 10066329
      Font.Height = -13
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      ExplicitLeft = 813
    end
    object DBText2: TDBText
      Left = 16
      Top = -4
      Width = 77
      Height = 30
      AutoSize = True
      DataField = 'NOME'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -21
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
    end
    object DBText3: TDBText
      Left = 16
      Top = 23
      Width = 57
      Height = 21
      AutoSize = True
      DataField = 'ABERTURA'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -16
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
    end
    object Image1: TImage
      Left = 362
      Top = 0
      Width = 66
      Height = 49
      Align = alRight
      Center = True
      Picture.Data = {
        
        454E44AE426082}
      OnClick = Image1Click
      ExplicitLeft = 337
    end
  end
  object DBLookupComboBox2: TDBLookupComboBox
    Left = 108
    Top = 159
    Width = 213
    Height = 33
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Segoe UI Semilight'
    Font.Style = []
    KeyField = 'CODIGO'
    ListField = 'NOME'
    ListSource = dsComandaD
    ParentFont = False
    TabOrder = 1
  end
  object Panel7: TPanel
    Left = 0
    Top = 276
    Width = 428
    Height = 25
    Align = alBottom
    BevelOuter = bvNone
    Color = 5592405
    ParentBackground = False
    TabOrder = 2
    DesignSize = (
      428
      25)
    object Label8: TLabel
      Left = 471
      Top = 22
      Width = 188
      Height = 17
      Anchors = [akTop, akRight]
      Caption = 'Relat'#243'rio de Vendas no Per'#237'odo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 10066329
      Font.Height = -13
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      ExplicitLeft = 813
    end
    object Splitter1: TSplitter
      Left = 0
      Top = 0
      Width = 13
      Height = 25
    end
    object Splitter2: TSplitter
      Left = 412
      Top = 0
      Width = 16
      Height = 25
      Align = alRight
      ExplicitLeft = 684
    end
    object Splitter3: TSplitter
      Left = 360
      Top = 0
      Width = 0
      Height = 25
      Align = alRight
      ExplicitLeft = 684
    end
    object Label9: TLabel
      Left = 360
      Top = 0
      Width = 52
      Height = 25
      Align = alRight
      Caption = 'F12 Sair'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -15
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      OnClick = Label9Click
      ExplicitHeight = 20
    end
    object Label3: TLabel
      Left = 13
      Top = 0
      Width = 99
      Height = 25
      Align = alLeft
      Caption = 'F10 - Transferir'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -15
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      OnClick = Label3Click
      ExplicitHeight = 20
    end
  end
  object DBLookupComboBox1: TDBLookupComboBox
    Left = 109
    Top = 92
    Width = 213
    Height = 33
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Segoe UI Semilight'
    Font.Style = []
    KeyField = 'CODIGO'
    ListField = 'NOME'
    ListSource = dsComandaO
    ParentFont = False
    TabOrder = 3
  end
  object qryComandaD: TFDQuery
    Connection = Dados.Conexao
    SQL.Strings = (
      'select * from comanda_pessoa'
      'where'
      'fk_comanda=:comanda and situacao='#39'A'#39
      'order by nome')
    Left = 33
    Top = 166
    ParamData = <
      item
        Name = 'COMANDA'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object qryComandaDCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      DisplayFormat = ',0.00'
    end
    object qryComandaDFK_COMANDA: TIntegerField
      FieldName = 'FK_COMANDA'
      Origin = 'FK_COMANDA'
      DisplayFormat = ',0.00'
    end
    object qryComandaDNOME: TStringField
      FieldName = 'NOME'
      Origin = 'NOME'
      Size = 30
    end
    object qryComandaDSITUACAO: TStringField
      FieldName = 'SITUACAO'
      Origin = 'SITUACAO'
      Size = 1
    end
    object qryComandaDTOTAL: TFMTBCDField
      FieldName = 'TOTAL'
      Origin = 'TOTAL'
      DisplayFormat = ',0.00'
      Precision = 18
      Size = 3
    end
    object qryComandaDPERCENTUAL: TFMTBCDField
      FieldName = 'PERCENTUAL'
      Origin = 'PERCENTUAL'
      DisplayFormat = ',0.00'
      Precision = 18
      Size = 3
    end
  end
  object dsComandaD: TDataSource
    DataSet = qryComandaD
    Left = 104
    Top = 168
  end
  object dsComandaO: TDataSource
    DataSet = qryComandaO
    Left = 104
    Top = 120
  end
  object qryComandaO: TFDQuery
    Connection = Dados.Conexao
    SQL.Strings = (
      'select * from comanda_pessoa'
      'where'
      'fk_comanda=:comanda and situacao='#39'A'#39
      'order by nome')
    Left = 33
    Top = 110
    ParamData = <
      item
        Name = 'COMANDA'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object qryComandaOCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      DisplayFormat = ',0.00'
    end
    object qryComandaOFK_COMANDA: TIntegerField
      FieldName = 'FK_COMANDA'
      Origin = 'FK_COMANDA'
      DisplayFormat = ',0.00'
    end
    object qryComandaONOME: TStringField
      FieldName = 'NOME'
      Origin = 'NOME'
      Size = 30
    end
    object qryComandaOSITUACAO: TStringField
      FieldName = 'SITUACAO'
      Origin = 'SITUACAO'
      Size = 1
    end
    object qryComandaOTOTAL: TFMTBCDField
      FieldName = 'TOTAL'
      Origin = 'TOTAL'
      DisplayFormat = ',0.00'
      Precision = 18
      Size = 3
    end
    object qryComandaOPERCENTUAL: TFMTBCDField
      FieldName = 'PERCENTUAL'
      Origin = 'PERCENTUAL'
      DisplayFormat = ',0.00'
      Precision = 18
      Size = 3
    end
  end
end
