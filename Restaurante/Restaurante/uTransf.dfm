object frmTransf: TfrmTransf
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsSizeToolWin
  Caption = 'Comanda Eletr'#244'nica'
  ClientHeight = 291
  ClientWidth = 418
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 93
    Top = 97
    Width = 184
    Height = 21
    Caption = 'Selecione Mesa de Destino'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Segoe UI Semilight'
    Font.Style = []
    ParentFont = False
  end
  object Panel5: TPanel
    Left = 0
    Top = 0
    Width = 418
    Height = 49
    Align = alTop
    BevelOuter = bvNone
    Color = 5592405
    ParentBackground = False
    TabOrder = 0
    DesignSize = (
      418
      49)
    object Label11: TLabel
      Left = 461
      Top = 22
      Width = 188
      Height = 17
      Anchors = [akTop, akRight]
      Caption = 'Relat'#243'rio de Vendas no Per'#237'odo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 10066329
      Font.Height = -13
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      ExplicitLeft = 813
    end
    object DBText2: TDBText
      Left = 16
      Top = -4
      Width = 83
      Height = 32
      AutoSize = True
      DataField = 'NOME'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -24
      Font.Name = 'Segoe UI Semilight'
      Font.Style = []
      ParentFont = False
    end
    object DBText3: TDBText
      Left = 16
      Top = 23
      Width = 69
      Height = 25
      AutoSize = True
      DataField = 'ABERTURA'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -19
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
    end
    object Image1: TImage
      Left = 352
      Top = 0
      Width = 66
      Height = 49
      Align = alRight
      Center = True
      Picture.Data = {
        
        454E44AE426082}
      OnClick = Image1Click
      ExplicitLeft = 337
    end
    object Label1: TLabel
      Left = 252
      Top = 6
      Width = 100
      Height = 32
      Caption = 'Transferir'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -24
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
  end
  object DBLookupComboBox2: TDBLookupComboBox
    Left = 92
    Top = 125
    Width = 213
    Height = 33
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Segoe UI Semilight'
    Font.Style = []
    KeyField = 'CODIGO'
    ListField = 'NOME'
    ListSource = dsMesasD
    ParentFont = False
    TabOrder = 1
  end
  object Panel7: TPanel
    Left = 0
    Top = 266
    Width = 418
    Height = 25
    Align = alBottom
    BevelOuter = bvNone
    Color = 5592405
    ParentBackground = False
    TabOrder = 2
    DesignSize = (
      418
      25)
    object Label8: TLabel
      Left = 461
      Top = 22
      Width = 188
      Height = 17
      Anchors = [akTop, akRight]
      Caption = 'Relat'#243'rio de Vendas no Per'#237'odo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 10066329
      Font.Height = -13
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      ExplicitLeft = 813
    end
    object Splitter1: TSplitter
      Left = 0
      Top = 0
      Width = 13
      Height = 25
    end
    object Splitter2: TSplitter
      Left = 402
      Top = 0
      Width = 16
      Height = 25
      Align = alRight
      ExplicitLeft = 684
    end
    object Splitter3: TSplitter
      Left = 350
      Top = 0
      Width = 0
      Height = 25
      Align = alRight
      ExplicitLeft = 684
    end
    object Label9: TLabel
      Left = 350
      Top = 0
      Width = 52
      Height = 25
      Align = alRight
      Caption = 'F12 Sair'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -15
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      OnClick = Label9Click
      ExplicitHeight = 20
    end
    object Label3: TLabel
      Left = 13
      Top = 0
      Width = 99
      Height = 25
      Align = alLeft
      Caption = 'F10 - Transferir'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -15
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      OnClick = Label3Click
      ExplicitHeight = 20
    end
  end
  object qryMesasD: TFDQuery
    Connection = Dados.Conexao
    SQL.Strings = (
      'select * from mesa'
      'where'
      'situacao='#39'L'#39
      'order by nome')
    Left = 57
    Top = 190
    object qryMesasDCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      DisplayFormat = ',0.00'
    end
    object qryMesasDNOME: TStringField
      FieldName = 'NOME'
      Origin = 'NOME'
      Size = 30
    end
    object qryMesasDQTD: TIntegerField
      FieldName = 'QTD'
      Origin = 'QTD'
      DisplayFormat = ',0.00'
    end
    object qryMesasDDATA: TSQLTimeStampField
      FieldName = 'DATA'
      Origin = '"DATA"'
    end
    object qryMesasDSITUACAO: TStringField
      FieldName = 'SITUACAO'
      Origin = 'SITUACAO'
      Size = 1
    end
    object qryMesasDFK_MOVIMENTO: TIntegerField
      FieldName = 'FK_MOVIMENTO'
      Origin = 'FK_MOVIMENTO'
      DisplayFormat = ',0.00'
    end
  end
  object dsMesasD: TDataSource
    DataSet = qryMesasD
    Left = 136
    Top = 184
  end
  object dsMesasO: TDataSource
    DataSet = Dados.qryMesas
    Left = 216
    Top = 192
  end
end
